...
Start with `./omadocs-run diagnostics --json` (or `omadocs diagnostics --json` after MIME installation). The panel’s Settings → Diagnostics section shows the same safe data. Helper events are fixed codes, not raw network logs.

| Symptom | Action |
| --- | --- |
| Production OAuth client not configured | This development build has no maintainer client yet. For development, import a Desktop JSON file outside the repository, then Connect account. Normal releases must bundle a validated maintainer public client ID. |
...
| Uploaded but browser failed/uncertain | Use Open in browser. Upload success is preserved and no new copy is created. |
...
| File picker unavailable | Install `zenity` to use Upload files/Browse, or use Open with → omadocs and drag-and-drop uploads. File selection runs outside the shell. |
| Opening a file for a paused account | The file appears as paused in Uploads. Choose Enable account to resume; no fallback account is used. |
