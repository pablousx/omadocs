...
- [ ] Inspect the installed panel, keyboard navigation, all six sections, theme changes, multiple monitors, long names, and bounded activity scrolling.
...
- [ ] Verify no symlinks, generated secrets, OAuth credential JSON, document fixtures containing private content, support archives, or databases are in the release tree.
...
- [ ] Validate the public-client-only PKCE/loopback flow with the actual production client before bundling `assets/oauth-client.json`.
